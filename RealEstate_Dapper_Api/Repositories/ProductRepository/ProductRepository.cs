...
namespace RealEstate_Dapper_Api.Repositories.ProductRepository
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            string query = "Select ProductID,Title,Price,City,District,Address,Type,SlugUrl,CoverImage, CategoryName,DealOfTheDay " +
      ...
      "from Product inner Join Category on Product.ProductCategory=Category.CategoryID where EmployeeId=@employeeID and ProductStatus = 1";
       ...
       "from Product inner Join Category on Product.ProductCategory=Category.CategoryID where EmployeeId=@employeeID and ProductStatus = 0";
        ...
        {
            string query = "Select ProductID,Title,Price,City,District,Address,Type,Description,CoverImage, CategoryName,DealOfTheDay,AdvertismentDate,SlugUrl " +
     "from Product inner Join Category on Product.ProductCategory=Category.CategoryID where ProductId = @productId";
        ...
        {
            string query = "Select ProductID,Title,Price,City,District,Address,Type,CoverImage, CategoryName,DealOfTheDay From Product inner Join Category on Product.ProductCategory=Category.CategoryID Where Title like '%" + searchKeyValue + "%' And ProductCategory=@propertyCategoryId And City=@city";
            var parameters = new DynamicParameters();
        ...
        public async Task<List<ResultProductWithCategoryDto>> GetProductsByDealOfDayTrueWithCategoryAsync()
        {
            string query = "Select top(6) ProductID,Title,Price,City,District,Address,Type,CoverImage, CategoryName,DealOfTheDay " +
        ...
        public async Task<List<ResultLast3ProductWithCategoryDto>> GetLastThreeProductsAsync()
        {
            string query = $"Select top(3) ProductID,Title,Price,City,District,CoverImage,ProductCategory, " +
     $"CategoryName,AdvertismentDate,Description " +
     $"from Product inner join Category on Product.ProductCategory = Category.CategoryID " +
     $" order by AdvertismentDate Desc";
